...
const { ensureAuth, authClient, profileMiddleware } = require('../auth');
...
router.get(baseUrl, ensureAuth, profileMiddleware,
    wrap(async (req, res) => {
      ...
      try {
        ...
        res.status(HttpStatus.OK).json(branchesRes.docs);
      } catch (err) {
        throw err;
 ...
 * Create a new branch
 */
router.post(baseUrl, ensureAuth, profileMiddleware,
    wrap(async (req, res) => {
 ...
 * Create a new review
 */
router.post(reviewUrl, ensureAuth, profileMiddleware,
    wrap(async (req, res) => {
      const branchId = req.params.id;
      try {
        const branch = await Branch.findById(branchId);
